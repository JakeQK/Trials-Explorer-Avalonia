...
using Trials_Explorer.Shared.Services;

namespace Trials_Explorer.Features.ArchiveExplorer.Services
{
    public class ArchiveService
    {
        ...
        private bool ParseArchiveStructure()
        {
            try
            {
                ...
                using(BinaryReader reader = new BinaryReader(ms))
                {
                    ...
                    {
                        Debug.WriteLine("Invalid file signature");
                        return false;
                    }
            ...
            {
                Debug.WriteLine($"Failed to parse archive structure: {ex.Message}");
        ...
        private bool ParseFilenameTable(ArchiveFile filenameTable)
        {
            ...
            {
                Debug.WriteLine($"Failed to parse filename table: {ex.Message}");
        ...
        private bool BuildFileHierarchy()
        {
            ...
            {
                Debug.WriteLine($"Failed to build file hierarchy: {ex.Message}");
                return false;
            }
        }
        #endregion
        ...
        public byte[] ExtractFile(ArchiveFile file)
        {
            ...
            {
                Debug.WriteLine("Archive is not loaded or file is null");
                return null;
            }
            ...
            try
            {
                ...
                using(BinaryReader reader = new BinaryReader(ms))
                {
                    ...
                    switch (file.ArchiveFileEntry.CompressionFlag)
                    {
                        ...
                        // zlib Compressed
                        case 1:
                            return ZlibService.Decompress(reader.ReadBytes((int)file.ArchiveFileEntry.CompressedSize));
                        // Trials Evolution (Xbox 360) compressed
                        case 4:
                            Debug.WriteLine("Trials Evolution: Xbox 360 compression unsupported", "Fusion Explorer");
                            return null;
                        ...
                        case 16:
                            Debug.WriteLine("Unsupported file type", "Currently unable to extract files from data_patch.pak\n\nExtracting raw data...");
            ...
            {
                Debug.WriteLine($"Failed to extract file: {ex.Message}");
        ...
        public void QuickExtractFile(ArchiveFile file)
        {
            try
            {
                ...
                if (data != null)
                {
                    ...
                    string fullPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Extracted Files", Path.GetFileName(_currentFilePath), file.DirectoryPath);
            ...
            {
                Debug.WriteLine($"Failed to extract file to extracts folder: {ex.Message}");
            }
        }
        ...
        public bool ReplaceFile(ArchiveFile file, byte[] data)
        {
            ...
            try
            {
                ...
                {
                    dataToWrite = ZlibService.Compress(data);
            ...
            {
                Debug.WriteLine($"Failed to replace file: {ex.Message}");
                return false;
            }
        }
        #endregion
        ...
        public async Task QuickExtractDirectoryAsync(ArchiveDirectory directory)
        {
            try
            {
                ...
                string rootPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Extracted Files", Path.GetFileName(_currentFilePath), directory.FullPath);
            ...
            catch (Exception ex)
            {
                ...
                Debug.WriteLine($"Failed to extract directory {directory.FullPath}: {ex.Message}");
            }
        }

        public async Task ExtractDirectoryToSelectedPathAsync(ArchiveDirectory directory)
        {
            // FileDialog should be removed an implemented in UI side
